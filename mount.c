...
static struct fuse_operations ufs_oper = {
	.init = ufs_init,		//初始化
	.getattr = ufs_getattr, //获取文件属性（包括目录的）

	.mknod = ufs_mknod,	  //创建文件
	.unlink = ufs_unlink, //删除文件
	.open = ufs_open,	  //无论是read还是write文件，都要用到打开文件
	.read = ufs_read,	  //读取文件内容
	.write = ufs_write,	  //修改文件内容
	//.release    = ufs_release,//和open相对，关闭文件

	.mkdir = ufs_mkdir,		//创建目录
	.rmdir = ufs_rmdir,		//删除目录
	.access = ufs_access,	//进入目录
	.readdir = ufs_readdir, //读取目录
};
